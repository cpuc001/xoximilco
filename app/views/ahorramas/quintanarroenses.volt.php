...
<div class="block block--hero <?= $classn ?>">
  ...
  <?php $this->partial('templates/_organism/carousel', ['slider' => $t->slider, 'id' => 'carouselHtml', 'async' => true, 'classn' => $classn]); ?>
    ...
    <?php foreach ($t->introduccion->parrafos as $parrafo) { ?>
        <p><?= $parrafo ?><p>
    <?php } ?>
    <?php foreach ($t->description->accordion as $descripcion) { ?>
        <div class="row">
            <div class="block">
                <h5><?= $descripcion->title ?></h5>
                <?php if (isset($descripcion->lista)) { ?>
                <ul>
                        <?php foreach ($descripcion->lista as $item) { ?>
                            <li><?= $item ?></li>
                        <?php } ?>
                </ul>
                <?php } ?>
                <?php if (isset($descripcion->figures)) { ?>
                    <section class="flex--wrap" data-items="3">
                    <?php foreach ($descripcion->figures as $item) { ?>
    ...
    <!--     <?php $this->partial('templates/_molecules/accordion', ['accordion' => $t->description->accordion]); ?>
    <button class="button button--block button-first-pay u-pull-right">Comprar Ahora</button>
    <div class="block block--mixed text--align-center u-cf">
	<h3>Venta Cruzada</h3>
	<div class="row">
		<div class="block--mixed-item">
			<img class="block--mixed-img" src="<?= $this->url->get('img/xcaret.jpg') ?>" alt="xcaret" class="img-responsive">
			<button class="button button--block button-first block--mixed_button"><i class="icon icon-add"></i> AGREGA PRODUCTO Y AHORRA MXN*</button>
		</div>
		<div class="block--mixed-item">
			<img class="block--mixed-img" src="<?= $this->url->get('img/xelha.jpg') ?>" alt="Xel-há" class="img-responsive">
			<button class="button button--block button-first block--mixed_button"><i class="icon icon-add"></i> AGREGA PRODUCTO Y AHORRA MXN*</button>
		</div>
	</div>
</div> -->
...
<?php } ?>
    ...
    <?php $this->partial('constant/requireCart', ['coupon' => 'QROO']); ?>
