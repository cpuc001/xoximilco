...
<div class="block block--hero <?= $classn ?>">
  ...
  <?php $this->partial('templates/_organism/carousel', ['slider' => $t->slider, 'id' => 'carouselHtml', 'async' => true, 'classn' => $classn]); ?>
    ...
    <?php foreach ($t->introduccion->items as $item) { ?>
        <h3><?= $item->title ?></h3>
        <?php if (isset($item->parrafos)) { ?>
            <?php foreach ($item->parrafos as $parrafo) { ?>
                <p><?= $parrafo ?></p>
            <?php } ?>
        <?php } ?>
        <?php if (isset($item->lista)) { ?>
            <ul>
            <?php foreach ($item->lista as $elemento) { ?>
                <?php if (isset($elemento->sublista)) { ?>
                    <li>
                        <?= $elemento->text ?>
                        <ul>
                        <?php foreach ($elemento->sublista as $subelemento) { ?>
                            <li><?= $subelemento ?></li>
                        <?php } ?>
                        </ul>
                    </li>
    ...
    <a class="button button--block button-second u-pull-right button-second" href="<?= $this->url->get($lang . '/' . $t->link->href) ?>"><?= $t->link->texto ?></a>
