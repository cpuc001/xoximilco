...
<svg viewBox="0 0 635.3 29.3"><defs><style>.a{fill:#fb0;}</style></defs><title>pleca</title><path class="a" d="M630.4,4.4H4.9V7.7H281.3A9.5,9.5,0,0,0,300.2,9a17.4,17.4,0,0,0,34.6,0,9.5,9.5,0,0,0,18.9-1.4H630.4Zm-340.7,11-1-.5-1.3-.8L286.1,13,285,11.7l-.8-1.3-.5-1V9h.4l1,.5,1.3.8,1.3,1.1,1.1,1.3.8,1.3.5,1v.4Zm8.2-6-.5,1-.8,1.3L295.5,13l-1.3,1.1-1.3.8-1,.5h-.4v-.4l.5-1,.8-1.3,1.1-1.3,1.3-1.1,1.3-.8,1-.5h.4Zm17.7,12.4-1.9-.8-2.3-1.5a21.4,21.4,0,0,1-2.3-2,21.2,21.2,0,0,1-2-2.3,17.9,17.9,0,0,1-1.5-2.3,13.9,13.9,0,0,1-.8-1.9l-.2-.8.8.2,1.9.8,2.3,1.5a21.2,21.2,0,0,1,2.3,2,21.5,21.5,0,0,1,2,2.3,18,18,0,0,1,1.5,2.3,14.1,14.1,0,0,1,.8,1.9l.2.8Zm14.9-10.9a13.9,13.9,0,0,1-.8,1.9,17.9,17.9,0,0,1-1.5,2.3,21.2,21.2,0,0,1-2,2.3,21.5,21.5,0,0,1-2.3,2L321.4,21l-1.9.8-.8.2.2-.8a14.1,14.1,0,0,1,.8-1.9,18.1,18.1,0,0,1,1.5-2.3,21.5,21.5,0,0,1,2-2.3,21.1,21.1,0,0,1,2.3-2l2.3-1.5,1.9-.8.8-.2Zm12.7,4.4-1-.5-1.3-.8L339.6,13l-1.1-1.3-.8-1.3-.5-1V9h.4l1,.5,1.3.8,1.3,1.1,1.1,1.3L343,14l.5,1v.4Zm8.2-6-.5,1-.8,1.3L349,13l-1.3,1.1-1.3.8-1,.5h-.4v-.4l.5-1,.8-1.3,1.1-1.3,1.3-1.1,1.3-.8,1-.5h.4Z"/></svg>
	...
	<div class="gallery">
	    <?php $v129426262899491410781iterator = $t->gallery; $v129426262899491410781incr = 0; $v129426262899491410781loop = new stdClass(); $v129426262899491410781loop->self = &$v129426262899491410781loop; $v129426262899491410781loop->length = count($v129426262899491410781iterator); $v129426262899491410781loop->index = 1; $v129426262899491410781loop->index0 = 1; $v129426262899491410781loop->revindex = $v129426262899491410781loop->length; $v129426262899491410781loop->revindex0 = $v129426262899491410781loop->length - 1; ?><?php foreach ($v129426262899491410781iterator as $image) { ?><?php $v129426262899491410781loop->first = ($v129426262899491410781incr == 0); $v129426262899491410781loop->index = $v129426262899491410781incr + 1; $v129426262899491410781loop->index0 = $v129426262899491410781incr; $v129426262899491410781loop->revindex = $v129426262899491410781loop->length - $v129426262899491410781incr; $v129426262899491410781loop->revindex0 = $v129426262899491410781loop->length - ($v129426262899491410781incr + 1); $v129426262899491410781loop->last = ($v129426262899491410781incr == ($v129426262899491410781loop->length - 1)); ?>
	        <?= (($v129426262899491410781loop->index0 % 3) == 0 ? '<div class="row">' : '') ?>
	            <div class="four columns">
	                <a class="item--gallery" href="<?= $this->url->get($image->href) ?>"><img class="img--responsive" src="<?= $this->url->get($image->src) ?>" alt="<?= $image->alt ?>"></a>
	            </div>
	        <?= (($v129426262899491410781loop->index % 3) == 0 ? '</div>' : '') ?>
	    <?php $v129426262899491410781incr++; } ?>
    </div>
